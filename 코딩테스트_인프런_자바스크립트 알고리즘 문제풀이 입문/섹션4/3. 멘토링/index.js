...
function main() {
    ...
    const studentNumList = new Array(N).fill(0).map(val => [])
    ...
    inputs
        ...
        value
            ...
            .map(Number)
            .forEach((num, index) => {
                studentNumList[num-1].push(index + 1)
            })
    })

    let cnt = 0
    studentNumList.forEach((nums, index)=>{
        for(let i=0; i<N; i++){
            if(i ===index)continue
            const mentor = Math.max(...nums)
            const mentee = Math.min(...studentNumList[i])
            console.log(index,"|||",mentor, mentee)
