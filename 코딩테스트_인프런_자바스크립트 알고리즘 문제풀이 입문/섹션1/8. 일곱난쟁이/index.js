...
function findDwarf(...height) {
    console.log(height)
    const randomNumList = makeList()
    let total = 0
    ...
    return findDwarf(...height)
}

function main() {
    const heights =[20, 7, 23, 19, 10, 15, 25, 8, 13]
    const res =findDwarf(...heights).sort().map(num => heights[num])
    console.log(res)
}
...
//     let answer = [...input];
//
//     const sum = input.reduce((sum, currVal) => {
//         return sum + currVal;
//     }, 0);
//
//     const diff = sum - 100;
